...
/**
 * Checks if a reference number (seuNumero) is generic/placeholder
 * (e.g., '0', '1', 'S/N', 'DOC', 'N/A', '-', '0000', etc.)
 */
...
export function detectBoletoDuplicate(
  ...
  const checkItemMatch = (item: { linhaDigitavel?: string; codigoBarras?: string; id?: string; seuNumero?: string; favorecidoCnpjCpf?: string }): boolean => {
    ...
    // Rule A: Full numeric barcode key match (>= 40 digits)
    if (candidateKey && itemKey && candidateKey === itemKey) return true;

    // Rule B: Match by specific non-generic reference number (seuNumero) + CNPJ
    if (hasValidRef) {
